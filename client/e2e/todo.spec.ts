...
async function clearAllItems(page: Page) {
  ...
  let removeButton = page.getByRole('button', { name: 'Remove Item' }).first();
  while (await removeButton.isVisible().catch(() => false)) {
    await removeButton.click();
    await page.waitForTimeout(300);
    removeButton = page.getByRole('button', { name: 'Remove Item' }).first();
  }
}
...
test.describe('Todo App', () => {
  ...
  test('should show empty state message when no items exist', async ({ page }) => {
