...
export class MysqlUserRepository implements UserRepository {
  ...
  private getPool(): mysql.Pool {
    if (!this.pool) {
      throw new Error('MysqlUserRepository not initialized. Call init() first.');
    }
    return this.pool;
  }
  ...
  async findByEmail(email: string): Promise<User | undefined> {
    return new Promise((resolve, reject) => {
      this.getPool().query('SELECT * FROM users WHERE email = ?', [email], (err, rows) => {
        if (err) return reject(err);
  ...
  async findById(id: string): Promise<User | undefined> {
    return new Promise((resolve, reject) => {
      this.getPool().query('SELECT * FROM users WHERE id = ?', [id], (err, rows) => {
        if (err) return reject(err);
        const results = rows as MysqlUserRow[];
        if (results.length === 0) return resolve(undefined);
        const row = results[0]!;
  ...
  async deleteUser(id: string): Promise<void> {
    return new Promise((resolve, reject) => {
      this.getPool().query('DELETE FROM users WHERE id = ?', [id], (err) => {
        if (err) return reject(err);
        resolve();
      });
    });
